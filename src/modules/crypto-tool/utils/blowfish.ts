...
// 简化的 S-box 初始化
for (let i = 1; i < 4; i++) {
  S_INIT[i] = S_INIT[0].map((v, j) => (v + i * 256 + j) >>> 0);
}
...
class Blowfish {
  ...
  private encryptPair(L: number, R: number): [number, number] {
    for (let i = 0; i < 16; i++) {
      L = (L ^ this.P[i]) >>> 0;
      R = (R ^ this.F(L)) >>> 0;
      [L, R] = [R, L];
  ...
  private decryptPair(L: number, R: number): [number, number] {
    for (let i = 17; i > 1; i--) {
      L = (L ^ this.P[i]) >>> 0;
      R = (R ^ this.F(L)) >>> 0;
      [L, R] = [R, L];
...
export const blowfishEncrypt = (plaintext: string, keyHex: string): string => {
  ...
  for (let i = 0; i < padded.length; i += 8) {
    ...
    const encrypted = bf.encryptBlock(block);
...
export const generateBlowfishKey = (length: number = 16): string => {
