...
const PORT_DATABASE: PortEntry[] = [
  ...
  { port: 25, protocol: 'TCP', service: 'SMTP', description: '简单邮件传输协议', riskLevel: 'high' },
  ...
  { port: 110, protocol: 'TCP', service: 'POP3', description: '邮局协议版本3', riskLevel: 'high' },
  ...
  { port: 143, protocol: 'TCP', service: 'IMAP', description: 'Internet 消息访问协议', riskLevel: 'high' },
  ...
  { port: 993, protocol: 'TCP', service: 'IMAPS', description: 'IMAP 安全传输', riskLevel: 'high' },
  { port: 995, protocol: 'TCP', service: 'POP3S', description: 'POP3 安全传输', riskLevel: 'high' },
  { port: 1080, protocol: 'TCP', service: 'SOCKS', description: 'SOCKS 代理', riskLevel: 'high' },
  { port: 1433, protocol: 'TCP', service: 'MS-SQL', description: 'Microsoft SQL Server database', riskLevel: 'high' },
  { port: 1521, protocol: 'TCP', service: 'Oracle', description: 'Oracle database', riskLevel: 'high' },
  { port: 2049, protocol: 'TCP/UDP', service: 'NFS', description: '网络文件系统', riskLevel: 'high' },
  { port: 3306, protocol: 'TCP', service: 'MySQL', description: 'MySQL database', riskLevel: 'high' },
  { port: 3389, protocol: 'TCP', service: 'RDP', description: '远程桌面协议', riskLevel: 'high' },
  { port: 5432, protocol: 'TCP', service: 'PostgreSQL', description: 'PostgreSQL database', riskLevel: 'high' },
  { port: 5900, protocol: 'TCP', service: 'VNC', description: '虚拟网络计算', riskLevel: 'high' },
  { port: 6379, protocol: 'TCP', service: 'Redis', description: 'Redis database', riskLevel: 'high' },
  { port: 8080, protocol: 'TCP', service: 'HTTP-Proxy', description: 'HTTP 代理', riskLevel: 'high' },
  { port: 8443, protocol: 'TCP', service: 'HTTPS-Alt', description: 'HTTPS 备用端口', riskLevel: 'high' },
  { port: 9200, protocol: 'TCP', service: 'Elasticsearch', description: 'Elasticsearch HTTP', riskLevel: 'high' },
  { port: 27017, protocol: 'TCP', service: 'MongoDB', description: 'MongoDB database', riskLevel: 'high' },
];
...
export function getHighFrequencyPorts(): PortEntry[] {
  const highFrequencyPortNumbers = [
    20, 21, 22, 23, 25, 53, 80, 110, 135, 139, 143, 443, 445, 993, 995, 1433, 1521, 3306, 3389, 5432, 5900, 6379, 8080, 8443, 9200, 27017,
  ];
  return PORT_DATABASE.filter(
    (entry) => highFrequencyPortNumbers.includes(entry.port) && entry.riskLevel === 'high'
  );
}

export function getAllPorts(): PortEntry[] {
  return PORT_DATABASE.map((entry) => ({ ...entry }));
}
