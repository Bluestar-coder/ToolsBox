...
describe('SubnetDividerTab', () => {
  ...
  it('should render CIDR input, mode selector, and calculate button', () => {
    ...
    expect(screen.getByPlaceholderText(t('modules.ipNetwork.subnet.cidrPlaceholder'))).toBeInTheDocument();
    expect(screen.getByText(t('modules.ipNetwork.subnet.modeByCount'))).toBeInTheDocument();
    expect(screen.getByText(t('modules.ipNetwork.subnet.modeByHosts'))).toBeInTheDocument();
    expect(screen.getByText(t('modules.ipNetwork.subnet.calculate'))).toBeInTheDocument();
  });
  ...
  it('should show error when calculating with empty input', () => {
    render(<SubnetDividerTab />);
    fireEvent.click(screen.getByText(t('modules.ipNetwork.subnet.calculate')));
    expect(screen.getByRole('alert')).toBeInTheDocument();
  });

  it('should show error for invalid CIDR input', () => {
    render(<SubnetDividerTab />);
    const input = screen.getByPlaceholderText(t('modules.ipNetwork.subnet.cidrPlaceholder'));
  ...
  it('should switch to byHosts mode and divide accordingly', () => {
    ...
    fireEvent.click(screen.getByText(t('modules.ipNetwork.subnet.modeByHosts')));
