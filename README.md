...
- **子网掩码转换工具**：
  ...
  - 网络信息计算：网络地址、广播地址、可用 IP 范围、通配符掩码
  - 子网规划：支持生成子网列表，展示每个子网的详细信息
  - 智能推荐：根据所需主机数自动推荐合适的子网掩码
  - 性能优化：使用 React 18 并发特性，交互响应时间 < 60ms
- **IP 归属地查询**：
  - 查询 IP 的国家、地区、城市、ISP、AS 号
  - 支持批量查询
  - 查询本机公网 IP
  - 智能代理检测：自动识别国内/国外 IP，选择最优查询策略
  - 使用多个免费 API（ip-api.com、ipapi.co、ip.sb）
- **端口号速查**：
  - 内置常见端口数据库
  - 按端口号或服务名搜索
